...
def run():
    ...
    for i, val in enumerate(sorted_lst):
        ...
        completion_time = burst_time + arrival_time if len(process_lst) == 0 else burst_time + \
            process_lst[i-1][3] if arrival_time < process_lst[i-1][3] else burst_time + \
            process_lst[i-1][3] + (arrival_time - process_lst[i-1][3])
        ...
        process_lst.append(
            [pname, arrival_time, burst_time, completion_time, TAT, WT])

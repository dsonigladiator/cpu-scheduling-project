...
def sortFunc(lst_process, lst_names, time_quantum):
    ...
    # To Find Out if the Processes Arrive at 0 or not
    minVal = min(copy_lst, key=lambda x: x[0])
    remVal = minVal[0]
    currentTime = remVal
    ...
    while copy_lst:
        ...
        if minVal[1] != 0:
            if minVal[1] < time_quantum:
                ...
                new_lst += [[minVal[0], minVal[1] -
                             time_quantum, currentTime + time_quantum]]
...
def run():
    ...
    sorted_process_lst, new_process_names = sortFunc(
        temp_process_lst, process_names, time_quantum)
    ...
    for process_name, process in zip(process_names, process_lst):
        ...
        average_TAT += TAT
        average_CT += completion_time
        new_process_lst.append(
            [process_name, process[0], process[1], completion_time, TAT, TAT - process[1]])

    pt = PrettyTable()
    pt.field_names = ["Process", "Arrival Time",
                      "Burst Time", "Completion Time", "TAT", "WT"]
    pt.add_rows(process_lst)
    print(pt)
    print(f"Average Waiting Time: {average_WT/num}")
    print(f"Average Turn Around Time: {average_TAT/num}")
    print(f"Average Completion Time: {average_CT/num}")

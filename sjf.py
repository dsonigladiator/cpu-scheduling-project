...
def run():
    ...
    for i, val in enumerate(sorted_process_lst):
        ...
        completion_time = burst + arrival if len(new_process_lst) == 0 else burst + \
            new_process_lst[i-1][3] if arrival < new_process_lst[i-1][3] else burst + \
            new_process_lst[i-1][3] + (arrival - new_process_lst[i-1][3])
